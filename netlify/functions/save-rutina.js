...
exports.handler = async (event) => {
  ...
  try {
    ...
    const respuesta = await llamarAppsScript('saveRutina', payload);
    ...
    console.error('Error save-rutina:', error);
    return {
      statusCode: 500,
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ status: 'error', message: 'No se pudo guardar la rutina' }),
    };
  }
};
